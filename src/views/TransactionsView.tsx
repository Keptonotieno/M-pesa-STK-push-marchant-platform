...
export const TransactionsView: React.FC<Props> = ({
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 animate-in fade-in duration-200">
      ...
      {/* Transactions Container: Cards on Mobile (md:hidden), Table on Desktop (hidden md:block) */}
                ...
                <div
                  ...
                  className={`p-4 space-y-3 transition ${
                    isSelected
                      ...
                      : 'hover:bg-slate-50/80 dark:hover:bg-slate-950/50'
                  }`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="rounded border-slate-300 dark:border-slate-700 text-emerald-600 focus:ring-emerald-500 cursor-pointer min-h-[20px] min-w-[20px]"
                      />
                      <div>
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <div className="mt-0.5">
                        {tx.status === 'SUCCESS' && (
                          <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold inline-block">
                            ● Successful
                          </span>
                        )}
                        {tx.status === 'PENDING' && (
                          <span className="px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-400 text-[10px] font-bold inline-block">
                            ● Pending PIN
                          </span>
                        )}
                        {isFailed && (
                          <span className="px-2 py-0.5 rounded-full bg-rose-500/15 text-rose-600 dark:text-rose-400 text-[10px] font-bold inline-block">
                            ● {tx.status}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-100 dark:border-slate-800/60 text-slate-500 dark:text-slate-400">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-[10px] font-bold bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
                        {tx.mpesaReceipt || 'Pending Receipt'}
                      </span>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-1">
                    <button
                      ...
                      className="px-3 py-1.5 min-h-[36px] rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
                    >
                    ...
                    {isFailed && (
                      <button
                        ...
                        className="px-3 py-1.5 min-h-[36px] rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
                      >
                        <RefreshCw className="w-3.5 h-3.5" />
                        <span>Retry STK</span>
                      </button>
          ...
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <button
                ...
                className="p-1 rounded-lg text-slate-400 hover:text-white transition cursor-pointer min-h-[36px] min-w-[36px] flex items-center justify-center"
              >
                ✕
              </button>
            </div>
